...
class ControlPanelGUI:
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(
            self.root, text="Drawing Machine Control Panel", font=("Arial", 16, "bold")
        )
        ...
        for i, (name, key) in enumerate(zip(motor_names, motor_keys)):
            ...
            ttk.Radiobutton(
                dir_frame, text="CW", variable=self.motor_directions[key], value="CW"
            ).pack(side="left", padx=5)
            ttk.Radiobutton(
                dir_frame, text="CCW", variable=self.motor_directions[key], value="CCW"
            ).pack(side="left", padx=5)
        ...
        ttk.Button(button_frame, text="Send Command", command=self.send_command).pack(
            side="left", padx=5
        )
    ...
    def send_command(self):
        ...
        command = {
            ...
            "motors": {
                key: {"rpm": var.get(), "dir": self.motor_directions[key].get()}
        ...
        self.status_var.set(
            f"Command sent: Canvas={command['motors']['canvas']['rpm']:.1f}RPM"
        )
