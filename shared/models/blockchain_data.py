...
class BlockchainDataValidationError(Exception):
    ...
    def __init__(self, message: str, field: Optional[str] = None, value: Optional[Union[str, int, float]] = None):
        self.field = field
        self.value = value
...
class ApiResponseTimes(BaseModel):
    ...
    def is_healthy(self) -> bool:
        ...
        return all(time < 5000 for time in [self.coinbase_ms, self.ethereum_rpc_ms, self.beacon_chain_ms])
...
class EthereumDataSnapshot(BaseModel):
    ...
    timestamp: float = Field(
        ...,
        description="Unix timestamp when data was captured"
    )
    ...
    def is_high_activity_epoch(self) -> bool:
        ...
        """
        return (
            self.gas_price_gwei > 50 and
            self.block_fullness_percent > 80 and
            self.blob_space_utilization_percent > 60
        )
    ...
    def get_market_condition(self) -> MarketCondition:
        ...
        is_volatile = (
            self.gas_price_gwei > 100 or 
            self.block_fullness_percent > 95 or
            self.blob_space_utilization_percent > 90
        )
    ...
    def is_valid_for_drawing(self) -> bool:
        ...
        """
        return (
            self.data_quality.is_acceptable_quality and
            self.data_quality.freshness_score >= 0.75 and
            self.api_response_times.is_healthy
        )
    ...
    def model_validate_json_safe(cls, json_data):
        ...
        if 'data_quality' in filtered_data and isinstance(filtered_data['data_quality'], dict):
            quality_computed = {'freshness_score', 'is_acceptable_quality'}
            filtered_data['data_quality'] = {
                k: v for k, v in filtered_data['data_quality'].items() 
                if k not in quality_computed
            }
        
        # Filter nested api_response_times computed fields
        if 'api_response_times' in filtered_data and isinstance(filtered_data['api_response_times'], dict):
            api_computed = {'average_response_time', 'is_healthy'}
            filtered_data['api_response_times'] = {
                k: v for k, v in filtered_data['api_response_times'].items() 
    ...
    def model_dump_json_safe(self, **kwargs):
        """Safe JSON dump that excludes computed fields."""
        exclude_computed = {
            'datetime_iso', 'is_high_activity_epoch', 'average_response_time', 
            'is_healthy', 'freshness_score', 'is_acceptable_quality'
        }
        exclude = kwargs.get('exclude', set())
        if isinstance(exclude, set):
            exclude = exclude.union(exclude_computed)
