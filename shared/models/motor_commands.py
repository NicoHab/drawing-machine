...
class MotorSafetyLimits(BaseModel):
    ...
    emergency_stop_rpm: float = Field(
        default=0.0, description="RPM value for emergency stop (always 0.0)"
    )
...
class MotorState(BaseModel):
    ...
    motor_name: MotorName = Field(..., description="Name identifier of the motor")
    ...
    is_enabled: bool = Field(
        default=True, description="Whether the motor is enabled for operation"
    )
...
class CommandExecutionStatus(BaseModel):
    ...
    def model_validate_json_safe(cls, json_data):
        ...
        root_computed_fields = {
            "all_motors_at_target",
            "any_motors_overheating",
        ...
        if "motor_responses" in filtered_data and isinstance(
            filtered_data["motor_responses"], list
        ):
            filtered_motor_responses = []
            for motor_state in filtered_data["motor_responses"]:
                if isinstance(motor_state, dict):
                    motor_state_computed = {
                        "velocity_error_rpm",
                        "is_at_target",
                        "is_overheating",
                        "status_summary",
                    ...
                    filtered_motor_state = {
                        ...
                        if k not in motor_state_computed
                    }
...
class SingleMotorCommand(BaseModel):
    ...
    velocity_rpm: float = Field(
        ..., ge=-200.0, le=200.0, description="Target velocity in RPM (-200 to 200)"
    )
        ...
        if hasattr(info, "data") and "direction" in info.data:
            ...
            if direction == MotorDirection.CLOCKWISE and v < 0:
                raise MotorCommandError(
                    "Clockwise direction requires positive velocity", command_value=v
                )
...
class MotorVelocityCommands(BaseModel):
    ...
    def validate_required_motors(self) -> "MotorVelocityCommands":
        ...
        if missing_motors:
            raise MotorCommandError(f"Missing required motors: {missing_motors}")

        extra_motors = provided_motors - required_motors
        if extra_motors:
            raise MotorCommandError(f"Unknown motors provided: {extra_motors}")
    ...
    def validate_safety_limits(self) -> "MotorVelocityCommands":
        ...
        for motor_name_str, command in self.motors.items():
            try:
                ...
                if not self.safety_limits.validate_rpm(
                    motor_name, command.velocity_rpm
                ):
    ...
    def apply_safety_override(self) -> "MotorVelocityCommands":
        ...
        for motor_name_str, command in self.motors.items():
            ...
            safe_motors[motor_name_str] = SingleMotorCommand(
                velocity_rpm=safe_velocity, direction=new_direction
            )
    ...
    def model_validate_json_safe(cls, json_data):
        ...
        if "source_data" in filtered_data and isinstance(
            filtered_data["source_data"], dict
        ):
            ...
            source_data = {
                k: v for k, v in source_data.items() if k not in eth_computed
            }

            # Filter nested data_quality computed fields
            if "data_quality" in source_data and isinstance(
                source_data["data_quality"], dict
            ):
                quality_computed = {"freshness_score", "is_acceptable_quality"}
                source_data["data_quality"] = {
                    k: v
                    for k, v in source_data["data_quality"].items()
                    if k not in quality_computed
                }

            # Filter nested api_response_times computed fields
            if "api_response_times" in source_data and isinstance(
                source_data["api_response_times"], dict
            ):
                api_computed = {"average_response_time", "is_healthy"}
                source_data["api_response_times"] = {
                    k: v
                    for k, v in source_data["api_response_times"].items()
