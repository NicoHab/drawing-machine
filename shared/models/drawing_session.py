...
class DrawingSessionError(Exception):
    ...
    def __init__(self, message: str, session_id: Optional[str] = None, mode: Optional[DrawingMode] = None):
...
class BlockchainModeConfig(BaseModel):
    ...
    gas_tracking_enabled: bool = Field(
        default=True,
        description="Enable gas price tracking for motor_pb control"
    )
    
    blob_tracking_enabled: bool = Field(
        default=True,
        description="Enable blob space tracking for motor_pcd control"
    )
    ...
    fallback_to_offline: bool = Field(
        default=True,
        description="Fall back to offline mode if data quality is poor"
    )
    ...
    api_timeout_seconds: float = Field(
        ...
        ge=1.0,
        le=30.0,
        description="API request timeout (1-30 seconds)"
    )
...
class ManualModeConfig(BaseModel):
    ...
    control_sensitivity: ControlSensitivity = Field(
        default=ControlSensitivity.MEDIUM,
        description="Control input sensitivity level"
    )
    ...
    velocity_scaling_factor: float = Field(
        ...
        ge=0.1,
        le=5.0,
        description="Velocity scaling multiplier (0.1-5.0)"
    )
    
    record_session: bool = Field(
        default=True,
        description="Record manual commands for later replay"
    )
    
    recording_file_path: Optional[str] = Field(
        default=None,
        description="Path to save recorded session data"
    )
    
    enable_safety_overrides: bool = Field(
        default=True,
        description="Enable automatic safety limit enforcement"
    )
    ...
    command_rate_limit_hz: float = Field(
        ...
        ge=1.0,
        le=100.0,
        description="Maximum command rate in Hz (1-100)"
    )
    ...
    joystick_deadzone: float = Field(
        ...
        ge=0.0,
        le=0.5,
        description="Joystick input deadzone (0.0-0.5)"
    )
    
    enable_haptic_feedback: bool = Field(
        default=False,
        description="Enable haptic feedback for control devices"
    )
...
class OfflineModeConfig(BaseModel):
    ...
    loop_playback: bool = Field(
        default=False,
        description="Loop the sequence continuously"
    )
    
    start_from_epoch: int = Field(
        default=0,
        ge=0,
        description="Epoch number to start playback from"
    )
    ...
    interpolate_missing_data: bool = Field(
        default=True,
        description="Interpolate motor commands for missing data points"
    )
    
    validate_sequence_integrity: bool = Field(
        default=True,
        description="Validate sequence file integrity before playback"
    )
    
    allow_speed_adjustment: bool = Field(
        default=True,
        description="Allow real-time speed adjustment during playback"
    )
...
class SessionStatistics(BaseModel):
    ...
    total_commands_sent: int = Field(
        default=0,
        ge=0,
        description="Total number of motor commands sent"
    )
    ...
    data_quality_scores: List[float] = Field(
        ...
        description="Historical data quality scores"
    )
    ...
    motor_temperature_readings: Dict[str, List[float]] = Field(
        ...
        description="Motor temperature readings over time"
    )
    
    power_consumption_watts: List[float] = Field(
        default_factory=list,
        description="Power consumption readings in watts"
    )
    
    safety_overrides_triggered: int = Field(
        default=0,
        ge=0,
        description="Number of safety override interventions"
    )
    ...
    def get_session_health_score(self) -> float:
        ...
        if self.data_quality_scores:
            average_data_quality = sum(self.data_quality_scores) / len(self.data_quality_scores)
        ...
        if self.api_response_times:
            average_api_response_time = sum(self.api_response_times) / len(self.api_response_times)
    ...
    def add_command_result(self, success: bool, latency_ms: float) -> None:
        ...
        self.average_command_latency_ms = (total_latency + latency_ms) / self.total_commands_sent
    ...
    def model_validate_json_safe(cls, json_data):
        ...
        computed_fields = {'command_success_rate', 'average_data_quality', 'average_api_response_time'}
        filtered_data = {k: v for k, v in data.items() if k not in computed_fields}
        return cls.model_validate(filtered_data)
    
    def model_dump_json_safe(self, **kwargs):
        """Safe JSON dump that excludes computed fields."""
        exclude_computed = {'command_success_rate', 'average_data_quality', 'average_api_response_time'}
        exclude = kwargs.get('exclude', set())
        if isinstance(exclude, set):
            exclude = exclude.union(exclude_computed)
        else:
...
class DrawingSessionConfig(BaseModel):
    ...
    mode: DrawingMode = Field(
        ...,
        description="Drawing operation mode"
    )
    ...
    end_time: Optional[datetime] = Field(
        default=None,
        description="Session end time (calculated from start + duration)"
    )
    
    blockchain_config: Optional[BlockchainModeConfig] = Field(
        default=None,
        description="Blockchain mode configuration"
    )
    ...
    offline_config: Optional[OfflineModeConfig] = Field(
        default=None,
        description="Offline mode configuration"
    )
    ...
    def estimated_epochs(self) -> int:
        ...
        if self.mode == DrawingMode.BLOCKCHAIN and self.blockchain_config:
            ...
            return int(total_seconds / self.blockchain_config.data_refresh_interval_seconds)
    ...
    def model_validate_json_safe(cls, json_data):
        ...
        if 'blockchain_config' in filtered_data and isinstance(filtered_data['blockchain_config'], dict):
            blockchain_computed = {'total_epoch_duration_minutes'}
            filtered_data['blockchain_config'] = {
                k: v for k, v in filtered_data['blockchain_config'].items() 
                if k not in blockchain_computed
            }
        
        # Filter nested manual_config computed fields
        if 'manual_config' in filtered_data and isinstance(filtered_data['manual_config'], dict):
            manual_computed = {'sensitivity_multiplier'}
            filtered_data['manual_config'] = {
                k: v for k, v in filtered_data['manual_config'].items() 
                if k not in manual_computed
            }
        
        # Filter nested offline_config computed fields
        if 'offline_config' in filtered_data and isinstance(filtered_data['offline_config'], dict):
            offline_computed = {'speed_multiplier', 'estimated_duration_minutes'}
            filtered_data['offline_config'] = {
                k: v for k, v in filtered_data['offline_config'].items() 
...
class DrawingSession(BaseModel):
    ...
    config: DrawingSessionConfig = Field(
        ...,
    ...
    total_epochs: int = Field(
        default=1575,
        ge=1,
        description="Total epochs in this session"
    )
    
    epochs_completed: int = Field(
        default=0,
        ge=0,
        description="Number of completed epochs"
    )
    ...
    is_active: bool = Field(
        default=False,
        description="Whether session is actively running"
    )
    
    current_motor_commands: Optional[MotorVelocityCommands] = Field(
        default=None,
        description="Current motor command set being executed"
    )
    ...
    error_log: List[str] = Field(
        default_factory=list,
        description="Session error messages"
    )
    ...
    def model_post_init(self, __context) -> None:
        ...
        if self.total_epochs == 1575:  # Default value
            if self.config.mode == DrawingMode.BLOCKCHAIN and self.config.blockchain_config:
                ...
                self.total_epochs = int(total_seconds / self.config.blockchain_config.data_refresh_interval_seconds)
            elif self.config.mode == DrawingMode.OFFLINE and self.config.offline_config:
                self.total_epochs = 1575  # Placeholder - would be calculated from sequence file
            else:
                # For manual/hybrid, estimate based on typical command rate
    ...
    def estimated_time_remaining_minutes(self) -> float:
        ...
        if self.config.mode == DrawingMode.BLOCKCHAIN and self.config.blockchain_config:
            remaining_seconds = remaining_epochs * self.config.blockchain_config.data_refresh_interval_seconds
            return remaining_seconds / 60.0
        else:
            # Fallback estimation
            progress_ratio = self.epochs_completed / self.total_epochs if self.epochs_completed > 0 else 0.1
            elapsed_minutes = (datetime.now() - self.config.start_time).total_seconds() / 60.0
            estimated_total_minutes = elapsed_minutes / progress_ratio if progress_ratio > 0 else self.config.duration_minutes
    ...
    def get_session_health_summary(self) -> Dict[str, Union[str, float]]:
        ...
        return {
            ...
            "command_success_rate": (self.session_statistics.successful_commands / self.session_statistics.total_commands_sent) if self.session_statistics.total_commands_sent > 0 else 0.0,
            "average_data_quality": (sum(self.session_statistics.data_quality_scores) / len(self.session_statistics.data_quality_scores)) if self.session_statistics.data_quality_scores else 0.0,
    ...
    def start_session(self) -> bool:
        ...
        self.config.end_time = self.config.start_time + timedelta(minutes=self.config.duration_minutes)
    ...
    def advance_epoch(self, motor_commands: Optional[MotorVelocityCommands] = None) -> bool:
    ...
    def update_blockchain_data(self, data: EthereumDataSnapshot) -> None:
        ...
        if self.config.mode in [DrawingMode.BLOCKCHAIN, DrawingMode.HYBRID]:
            ...
            self.session_statistics.add_data_quality_score(data.data_quality.overall_quality_score)
    ...
    def switch_mode(self, new_mode: DrawingMode, new_config: Optional[Dict[str, Any]] = None) -> bool:
        ...
        if (self.config.mode, new_mode) not in allowed_switches:
            self.add_error(f"Mode switch from {self.config.mode} to {new_mode} not allowed")
            return False
        ...
        try:
            ...
            if new_config:
                if new_mode == DrawingMode.BLOCKCHAIN and 'blockchain_config' in new_config:
                    self.config.blockchain_config = BlockchainModeConfig(**new_config['blockchain_config'])
                elif new_mode == DrawingMode.MANUAL and 'manual_config' in new_config:
                    self.config.manual_config = ManualModeConfig(**new_config['manual_config'])
    ...
    def _initialize_offline_mode(self) -> None:
        ...
        if not Path(self.config.offline_config.sequence_file_path).exists():
            raise DrawingSessionError(f"Sequence file not found: {self.config.offline_config.sequence_file_path}")
    ...
    def _filter_computed_fields_recursive(cls, data):
        ...
        root_computed_fields = {'progress_percentage', 'estimated_time_remaining_minutes'}
        ...
        for key, value in filtered.items():
            if key == 'config' and isinstance(value, dict):
                ...
                filtered[key] = [cls._filter_computed_fields_recursive(item) if isinstance(item, dict) else item for item in value]
    ...
    def _filter_config_computed_fields(cls, config_data):
        ...
        filtered_config = {k: v for k, v in config_data.items() if k not in config_computed_fields}
        
        # Filter blockchain_config computed fields
        if 'blockchain_config' in filtered_config and isinstance(filtered_config['blockchain_config'], dict):
            blockchain_computed = {'total_epoch_duration_minutes'}
            filtered_config['blockchain_config'] = {
                k: v for k, v in filtered_config['blockchain_config'].items() 
                if k not in blockchain_computed
            }
        
        # Filter manual_config computed fields
        if 'manual_config' in filtered_config and isinstance(filtered_config['manual_config'], dict):
            manual_computed = {'sensitivity_multiplier'}
            filtered_config['manual_config'] = {
                k: v for k, v in filtered_config['manual_config'].items() 
                if k not in manual_computed
            }
        
        # Filter offline_config computed fields
        if 'offline_config' in filtered_config and isinstance(filtered_config['offline_config'], dict):
            offline_computed = {'speed_multiplier', 'estimated_duration_minutes'}
            filtered_config['offline_config'] = {
                k: v for k, v in filtered_config['offline_config'].items() 
                if k not in offline_computed
            }
    ...
    def _filter_session_stats_computed_fields(cls, stats_data):
        ...
        stats_computed_fields = {'command_success_rate', 'average_data_quality', 'average_api_response_time'}
...
HYBRID_SESSION_EXAMPLE = {
    ...
    "blockchain_config": {
        "data_refresh_interval_seconds": 15.0
    },
    "manual_config": {
