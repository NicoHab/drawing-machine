...
class DrawingMachineFileHandler(FileSystemEventHandler):
    ...
    def __init__(
        self, callback: Callable[[FileChangeEvent], None], debounce_delay: float = 2.0
    ):
        """
...
class TestExecutor:
    ...
    def __init__(self, project_root: Path):
        ...
        self.test_mappings = {
            "shared": ["tests/unit/test_foundational_models.py"],
            "edge": [
    ...
    def run_tests(
        ...
        report_file = (
            self.reports_dir
            / f"pytest_report_{timestamp.strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        print(
            f"{Fore.BLUE}Running tests: {' '.join(cmd[3:])}"
        )  # Skip 'python -m pytest'
    ...
    def display_test_result(self, result: TestResult):
        ...
        if result.coverage_percent is not None:
            coverage_color = (
                Fore.GREEN if result.coverage_percent >= 80 else Fore.YELLOW
            )
...
class FileWatcher:
    ...
    def __init__(
        ...
        self.test_executor = (
            TestExecutor(self.project_root) if enable_auto_tests else None
        )

        # Create file handler
        self.file_handler = DrawingMachineFileHandler(
            callback=self.handle_file_change, debounce_delay=debounce_delay
        )

        print(
            f"{Fore.CYAN}FileWatcher initialized for Drawing Machine TDD Infrastructure"
        )
        print(f"{Fore.CYAN}Project root: {self.project_root}")
        print(f"{Fore.CYAN}Debounce delay: {debounce_delay} seconds")
        print(
            f"{Fore.CYAN}Auto-test execution: {'ENABLED' if enable_auto_tests else 'DISABLED'}"
        )
    ...
    def handle_file_change(self, event: FileChangeEvent):
        ...
        area_desc = self.file_handler.project_areas.get(
            event.project_area, f"Other ({event.project_area})"
        )
        ...
        for test_path in test_paths:
            ...
            try:
                ...
                result = self.test_executor.run_tests(
                    test_path=test_path, with_coverage=True
                )
                ...
                if len(test_paths) > 1:
                    ...
                    print(
                        f"{Fore.CYAN}Test suite {status}: {test_path} "
    ...
    def start_watching(self):
        ...
        try:
            ...
            if not self.validate_project_structure():
                print(
                    f"{Fore.RED} Cannot start FileWatcher - invalid project structure"
                )
                return False
    ...
    def stop_watching(self):
        ...
        print(
            f"{Fore.GREEN} FileWatcher stopped. Thank you for using Drawing Machine TDD!"
        )
...
def test_file_watcher():
    ...
    if monitored_paths:
        print(
            f"{Fore.GREEN} Monitored paths detection: PASSED ({len(monitored_paths)} paths)"
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test", action="store_true", help="Run FileWatcher functionality tests"
    )
    parser.add_argument(
        "--demo", action="store_true", help="Demonstrate file change detection"
    )
    parser.add_argument(
        "--debounce",
        type=float,
    ...
    parser.add_argument(
        "--run-test", type=str, help="Run a specific test file manually and exit"
    )
    ...
    if args.test:
        ...
        watcher = FileWatcher(
            debounce_delay=args.debounce, enable_auto_tests=enable_auto_tests
        )
