...
def test_file_watcher_integration():
    ...
    try:
        ...
        test_file.write_text("# Modified test module\ndef hello():\n    return 'modified world'\n")
        ...
        relevant_events = [
            ...
            if "test_module.py" in str(e.file_path)
        ]
        ...
        for optional in optional_events:
            if optional in event_types:
                ...
                print(f"[NOTE] Optional event not detected: {optional} (this is normal)")
        
        # Verify ignored file was not processed
        ignored_events = [
            e for e in detected_events 
            if "test.pyc" in str(e.file_path)
        ]
...
def test_debouncing():
    ...
    if len(detected_events) == 1:
        print("[PASS] Debouncing working correctly - 5 rapid changes resulted in 1 event")
        return True
    else:
        print(f"[FAIL] Debouncing failed - expected 1 event, got {len(detected_events)}")
        return False
