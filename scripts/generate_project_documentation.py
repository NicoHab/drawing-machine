...
"""
        ...
        (self.templates_dir / "architecture.md.j2").write_text(architecture_template.strip())
    ...
    def analyze_function(self, node: ast.FunctionDef, file_path: Path, is_async: bool = False) -> None:
    ...
    def extract_fastapi_endpoints(self, content: str, file_path: Path) -> None:
        ...
        for method, path, function_name in matches:
            ...
            description = doc_match.group(1).strip() if doc_match else "No description available"
    ...
    def analyze_component_relationships(self) -> Dict[str, Any]:
        ...
        components["TDD Infrastructure"] = {
            ...
            "used_by": ["All development workflows", "CI/CD pipeline", "Claude Code sessions"]
        }
        ...
        components["Project Templates"] = {
            ...
            "used_by": ["New project creation", "Component scaffolding", "Development onboarding"]
        }
        ...
        for module_name, module_data in modules.items():
            if module_data["classes"] or module_data["functions"]:
                content = template.render(
                    ...
                    module_description=module_data["module_info"].docstring or "No description available",
                    timestamp=datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    ...
    def integrate_with_git(self) -> bool:
        ...
        try:
            ...
            if result.stdout.strip():
                ...
                commit_message = f"docs: Update comprehensive project documentation\n\n" \
            ...
            class DocUpdateHandler(FileSystemEventHandler):
                ...
                def on_modified(self, event):
                    ...
                    if (file_path.suffix == '.py' and 
                        any(part in str(file_path) for part in ['shared', 'scripts', 'tests'])):
...
Features:
  ...
  - Real-time updates with FileWatcher integration
    ...
    parser.add_argument(
        "--full",
        action="store_true",
        help="Generate complete documentation (default)"
    )
    ...
    parser.add_argument(
        "--modules-only",
        action="store_true",
        help="Generate only module documentation"
    )
    ...
    print(f"Target: {'Full documentation' if args.full or not any([args.api_only, args.architecture, args.tdd_only, args.modules_only, args.watch]) else 'Partial documentation'}")
